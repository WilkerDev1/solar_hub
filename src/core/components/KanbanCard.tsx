...
export default function KanbanCard({
  ...
  if (index === -1) {
    return (
      <div
        onClick={onClick}
        className={`bg-[#1c1c21] border-t-4 ${borderAccentColor} border-l border-r border-b border-zinc-800 p-4 rounded-none flex flex-col justify-between hover:border-zinc-500 transition-all select-none relative ${isCompleted ? 'opacity-65' : ''}`}
      >
        {cardInner}
      </div>
    );
    ...
    <Draggable draggableId={task.id} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          onClick={onClick}
          className={`bg-[#1c1c21] border-t-4 ${borderAccentColor} border-l border-r border-b border-zinc-800 p-4 rounded-none flex flex-col justify-between hover:border-zinc-500 transition-all select-none relative ${
