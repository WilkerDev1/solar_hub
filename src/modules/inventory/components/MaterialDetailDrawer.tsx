      ...
      <div className="fixed top-16 bottom-0 right-0 w-full md:w-[500px] bg-[#051424] border-l border-zinc-800 shadow-2xl flex flex-col z-50 transform transition-transform duration-300 animate-in slide-in-from-right duration-300">
        ...
        <header className="sticky top-0 bg-[#051424]/80 backdrop-blur-sm border-b border-zinc-800 p-4 flex justify-between items-center z-10 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-mono text-sm font-bold tracking-wider uppercase text-[#d4e4fa]">
          ...
          <div className="flex items-center gap-2">
            ...
            <RequirePermission action="inventory:write">
              <button
                ...
                className="bg-[#fbbf24] hover:bg-[#d97706] text-black font-mono text-[10px] font-bold uppercase tracking-wider h-[32px] px-4 rounded transition-colors flex items-center gap-1.5 cursor-pointer"
              >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 text-left [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-[#051424] [&::-webkit-scrollbar-thumb]:bg-[#3f465c] [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-[#adb4ce]">
          
          {activeTab === 'info' ? (
            <>
              {/* Title & SKU block */}
              <div className="space-y-1 pb-1">
                <h1 className="text-xl font-bold text-[#d4e4fa] tracking-tight leading-tight">
                  {selectedItem.name}
                </h1>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`font-mono text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-[2px] border ${
                    currentStock > 0
                  ...
                  <p className="font-mono text-xs text-[#d3c5ac]">{selectedItem.sku}</p>
                </div>
              </div>

              {/* Image Section */}
              <div className="space-y-3">
                <div className="bg-[#122131] border border-[#334155] rounded-lg p-1 h-[240px] flex items-center justify-center relative overflow-hidden shadow-inner">
                  {resolvedMainImage ? (
                    <img 
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  {Array.from({ length: Math.max(3 - images.slice(0, 3).length, 0) }).map((_, i) => (
                    <div 
                      key={`empty-${i}`} 
                      className="aspect-square rounded border border-[#334155]/40 bg-[#122131]/40 flex items-center justify-center text-[#d3c5ac]/20 select-none"
                    >
                      <Package className="h-5 w-5" />
                    </div>
                  ))}
                  {/* 4th slot: Camera icon placeholder */}
                  <div className="aspect-square rounded border border-[#334155] border-dashed flex items-center justify-center bg-[#122131]/60 text-[#d3c5ac]/40">
              ...
              <div className="bg-[#122131] border border-[#334155] rounded-lg p-4">
                <h3 className="font-mono text-[10px] font-bold tracking-wider uppercase text-[#d3c5ac] mb-3 flex items-center gap-2">
                  <Package className="h-4.5 w-4.5 text-[#fbbf24]" />
                  <span>Stock e Inventario</span>
                </h3>
                <div className="grid grid-cols-3 gap-2 font-mono text-center">
                  <div>
                    <p className="text-[9px] font-bold text-[#d3c5ac]/70 uppercase">Actual</p>
                    <p className="text-lg font-bold text-[#d4e4fa]">{currentStock.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-[#d3c5ac]/70 uppercase">Mínimo</p>
                    <p className="text-lg font-bold text-[#d4e4fa]">{minStock.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-[#d3c5ac]/70 uppercase">Unidad</p>
                    <p className="text-lg font-bold text-[#d4e4fa] uppercase">{selectedItem.unit}</p>
                  </div>
                </div>
                {/* Dynamically colored progress bar */}
                <div className="w-full bg-[#334155] h-1.5 mt-4 rounded-full overflow-hidden">
                  <div 
              ...
              <div className="space-y-4">
                ...
                <div className="bg-[#122131] border border-[#334155] rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between items-center pb-1.5 border-b border-[#334155]/40">
                      <span className="text-xs text-[#d3c5ac]">Categoría</span>
                      <span className="font-mono text-xs text-[#d4e4fa]">{category ? category.name : 'N/A'}</span>
                    </div>
                    <div className="flex justify-between items-center pb-1.5 border-b border-[#334155]/40">
                      <span className="text-xs text-[#d3c5ac]">Costo</span>
                      <span className="font-mono text-xs text-[#d4e4fa] font-bold">
                        ${selectedItem.cost.toFixed(2)}/{selectedItem.unit}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-[#d3c5ac]">Reorden</span>
                ...
                <div className="bg-[#122131] border border-[#334155] rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between items-center pb-1.5 border-b border-[#334155]/40">
                      <span className="text-xs text-[#d3c5ac]">Embalaje</span>
                      <span className="font-mono text-xs text-[#d4e4fa] uppercase font-semibold">
                        {selectedItem.packaging || 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-[#d3c5ac]">Peso</span>
                      <span className="font-mono text-xs text-[#d4e4fa] font-bold">
                ...
                {/* Description */}
                <div className="bg-[#122131] border border-[#334155] rounded-lg p-4">
                  <h3 className="font-mono text-[10px] font-bold tracking-wider uppercase text-[#d3c5ac] mb-2 flex items-center gap-2">
                    <FileText className="h-4.5 w-4.5 text-[#fbbf24]" />
                ...
                {/* Suppliers */}
                <div className="bg-[#122131] border border-[#334155] rounded-lg p-4 mb-4">
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="font-mono text-[10px] font-bold tracking-wider uppercase text-[#d3c5ac] flex items-center gap-2">
                      <Store className="h-4.5 w-4.5 text-[#fbbf24]" />
                  ...
                  <div className="space-y-2">
                    {selectedItem.providers && selectedItem.providers.length > 0 ? (
                      selectedItem.providers.map((pStr, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex justify-between items-center p-2 bg-[#1c2b3c]/30 rounded border border-[#334155]/20">
                            <div className="flex items-center gap-2">
                              <Store className="h-4 w-4 text-[#d3c5ac]" />
                              <span className="text-xs font-semibold text-[#d4e4fa]">{parsed.name}</span>
                            </div>
                      ...
                      <div className="text-xs italic text-[#d3c5ac]/50 py-2">
            ...
            <div className="max-w-xl mx-auto py-2 text-left">
              {loadingTransactions ? (
                  ...
                  {itemTransactions.map((tx) => {
                    ...
                    return (
                      <div key={tx.id} className="relative">
                        {/* Timeline dot */}
                        <div className="absolute -left-[31px] top-1 h-2.5 w-2.5 rounded-full bg-[#122131] ring-4 ring-[#051424] flex items-center justify-center">
                        ...
                        <div className="space-y-1.5">
                          <span className="text-[9px] bg-[#122131] border border-[#334155]/60 text-[#d3c5ac]/80 px-2 py-0.5 rounded font-mono">
                            {dateStr}
                          </span>
                          <div className="text-xs font-bold text-[#d4e4fa] leading-snug">
                            {isEntry 
                          ...
                          {tx.reason && <p className="text-xs text-[#d3c5ac]/80 leading-relaxed">{tx.reason}</p>}
                          <div className="flex items-center gap-1 text-[9px] text-[#d3c5ac]/60 font-mono">
