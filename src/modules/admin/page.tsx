    ...
    <RequirePermission action="admin:*" fallback={
      ...
      {view === 'menu' ? (
        ...
        <div className="space-y-6 max-w-5xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center gap-3 border-b border-zinc-700 pb-5 text-left">
            <button
              onClick={() => setView('menu')}
              className="h-9 w-9 rounded-none bg-zinc-900 border border-zinc-700 hover:bg-zinc-800 text-zinc-400 hover:text-white flex items-center justify-center transition-colors cursor-pointer"
            >
          ...
          <div className="bg-zinc-800 border border-zinc-700 rounded-none p-6 space-y-6 text-left">
            <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  <label className="text-zinc-400 text-xs font-mono uppercase tracking-wider block">Seleccionar Rol</label>
                  <div className="flex items-center gap-2">
                    <select
                      value={selectedRole}
                      onChange={(e) => setSelectedRole(e.target.value)}
                      className="flex-1 bg-zinc-900 border border-zinc-700 rounded-none text-sm p-2.5 text-white h-11 focus:border-emerald-500 outline-none"
                    >
                      {templates.length === 0 ? (
                        <>
                    ...
                    <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                      <DialogTrigger className="h-11 px-3 bg-zinc-900 border border-zinc-700 hover:bg-zinc-800 rounded-none shrink-0 flex items-center justify-center cursor-pointer">
                        <Plus className="h-4 w-4 text-zinc-300" />
                      </DialogTrigger>
                      <DialogContent className="bg-zinc-800 border border-zinc-700 rounded-none text-white">
                        <DialogHeader>
                        ...
                        <div className="py-4 space-y-2">
                          ...
                          <input 
                            ...
                            className="w-full bg-zinc-900 border border-zinc-700 rounded-none p-3 text-white text-sm focus:border-emerald-500 focus:outline-none"
                            placeholder="Ej: Asesor Legal"
                          />
                        </div>
                        <DialogFooter>
                          <Button onClick={handleCreateRole} disabled={savingTemplate || !newRoleName.trim()} className="bg-emerald-600 hover:bg-emerald-500 text-white rounded-none cursor-pointer">
                ...
                <div className="bg-zinc-900 border border-zinc-700 p-4 rounded-none text-xs space-y-2">
              ...
              <div className="lg:col-span-2 space-y-4">
                <span className="text-zinc-400 text-xs font-mono uppercase tracking-wider block">Permisos Disponibles</span>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-56 overflow-y-auto pr-2 scrollbar-thin">
                  {allPerms.map((perm) => {
                    const isChecked = rolePerms.includes(perm.action);
                    return (
                      <label
                        key={perm.id}
                        className="flex items-start gap-2.5 p-2 bg-zinc-900 border border-zinc-700 hover:border-zinc-550 rounded-none cursor-pointer transition-colors"
                      >
                        <input
                          type="checkbox"
                          checked={isChecked}
                ...
                <div className="flex justify-end pt-2">
                  <Button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold h-11 px-5 rounded-none text-sm flex items-center gap-2 cursor-pointer"
                  >
                    {savingTemplate ? (
