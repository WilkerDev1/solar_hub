...
export default function KanbanCard({
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'pendiente':
        return <span className="bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">Por Hacer</span>;
      case 'en_progreso':
        return <span className="bg-amber-500/10 text-amber-450 border border-amber-500/20 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">En Progreso</span>;
      case 'bloqueada':
        return <span className="bg-rose-500/10 text-rose-450 border border-rose-500/20 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded animate-pulse">Bloqueada</span>;
      case 'completada':
        return <span className="bg-emerald-500/10 text-emerald-450 border border-emerald-500/20 text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">Hecha</span>;
      default:
        return null;
    }
  };
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-1.5">
        <div className="flex gap-1.5 flex-wrap items-center">
          ...
          {projects && (
            <span className="bg-zinc-900 border border-zinc-800 text-[9px] font-semibold px-2 py-0.5 rounded text-zinc-400 flex items-center gap-1">
  ...
  // If index is negative, it's rendered outside of hello-pangea-dnd (so no Draggable context)
  if (index < 0) {
    return (
      <div
        onClick={onClick}
        className={`bg-[#1c1c21] border-t-4 ${borderAccentColor} border-l border-r border-b border-zinc-800 p-4 rounded-none flex flex-col justify-between hover:border-zinc-700 transition-all select-none relative break-inside-avoid ${
    ...
    <Draggable draggableId={task.id} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          onClick={onClick}
          className={`bg-[#1c1c21] border-t-4 ${borderAccentColor} border-l border-r border-b border-zinc-800 p-4 rounded-none flex flex-col justify-between hover:border-zinc-700 transition-all select-none relative ${
