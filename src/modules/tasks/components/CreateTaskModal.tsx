...
import { 
  Plus, X, User, Folder, Briefcase, Flag, Calendar, 
...
interface CreateTaskModalProps {
  ...
  createForm: {
    title: string;
    description: string;
    origin: 'proyecto' | 'almacen' | 'administracion';
    task_type: 'check' | 'entregable' | 'reporte' | 'evidencia';
    assigned_to: string;
    project_id: string;
    area: 'general' | 'legal' | 'almacen' | 'operaciones' | 'administracion';
...
export default function CreateTaskModal({
  ...
  const [activePopover, setActivePopover] = useState<'assignee' | 'project' | 'area' | 'priority' | 'due_date' | 'options' | null>(null);
  ...
  const linkedProject = projects.find(proj => proj.id === createForm.project_id);
      ...
      <div className="bg-zinc-950 border border-zinc-850 rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col shadow-2xl relative z-50">
        <div className="p-4 border-b border-zinc-850 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2 text-emerald-400">
            ...
            <h3 className="font-bold text-xs uppercase tracking-wide">Nueva Tarea</h3>
          </div>
          <button 
            type="button"
        ...
        <form onSubmit={handleCreateSubmit} className="p-5 overflow-y-auto space-y-4 text-left flex-1 min-h-0">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full bg-zinc-900/50 border border-zinc-800 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-emerald-500 focus:bg-zinc-900 placeholder-zinc-600 font-semibold"
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            ...
            {createForm.priority && (
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-zinc-900 border border-zinc-800 ${
                createForm.priority === 'alta' ? 'text-rose-400' :
          ...
          {showAdvanced && (
            <div className="pt-3 border-t border-zinc-900 space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {/* 2. Obra / Proyecto Button + Popover */}
                <div className="relative">
                  <button
                    type="button"
                  ...
                  {activePopover === 'project' && (
                    <div className="absolute left-0 mt-1.5 w-64 bg-zinc-900 border border-zinc-855 rounded-xl p-3 shadow-2xl z-50 flex flex-col gap-2">
                      <div className="flex items-center gap-1.5 bg-zinc-955 border border-zinc-800 px-2 py-1 rounded-lg">
                        <Search className="h-3.5 w-3.5 text-zinc-500" />
                        <input
                          type="text"
                      ...
                      <div className="max-h-40 overflow-y-auto space-y-1 pr-1 scrollbar-thin scrollbar-thumb-zinc-800">
                        <button
                          ...
                          className={`w-full text-left text-[11px] font-semibold p-2 hover:bg-zinc-850 rounded-lg block truncate ${
                            !createForm.project_id ? 'bg-zinc-850 text-emerald-400' : 'text-zinc-400'
                          }`}
                        >
                          Sin vincular (Huérfana)
                        </button>
                        {filteredProjects.map(proj => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              setCreateForm({ ...createForm, project_id: proj.id });
                              setActivePopover(null);
                            }}
                            className={`w-full text-left text-[11px] font-semibold p-2 hover:bg-zinc-850 rounded-lg block truncate ${
                              createForm.project_id === proj.id ? 'bg-zinc-850 text-emerald-400' : 'text-zinc-300'
                            }`}
                          >
                            {proj.name}
                          </button>
                    ...
                    <div className="absolute left-0 mt-1.5 w-40 bg-zinc-900 border border-zinc-855 rounded-xl p-2 shadow-2xl z-50 flex flex-col gap-1">
                      {[
                        ...
                        <button
                          ...
                          className={`w-full text-left text-[11px] font-semibold p-2 hover:bg-zinc-850 rounded-lg flex items-center justify-between ${
                            createForm.priority === item.key ? 'bg-zinc-850 text-emerald-400' : 'text-zinc-300'
                          }`}
                        >
                ...
                {/* 6. Opciones Button (Task type & requires audit) + Popover */}
                <div className="relative">
                  <button
                    type="button"
                  ...
                  {activePopover === 'options' && (
                    <div className="absolute right-0 md:left-0 mt-1.5 w-60 bg-zinc-900 border border-zinc-855 rounded-xl p-3.5 shadow-2xl z-50 flex flex-col gap-3">
                      ...
                      <div className="pt-2 border-t border-zinc-800 flex items-center gap-2">
                        ...
                        />
                        <label 
                          htmlFor="popover-requires-audit" 
