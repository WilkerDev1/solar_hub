    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-2 space-y-6">
        ...
        <div className="h-48 rounded-none overflow-hidden border border-zinc-700 relative group">
          ...
          <label className="absolute top-4 right-4 bg-zinc-900 border border-zinc-700 hover:bg-zinc-800 text-white rounded-none px-3 py-2 text-xs font-bold flex items-center gap-1.5 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity z-10 select-none">
        ...
        {/* Gallery Section */}
        <div className="bg-zinc-800 border border-zinc-700 p-6 rounded-none space-y-5 text-left">
          ...
          {/* Carousel */}
          {images.length === 0 ? (
            <div className="py-8 text-center italic text-zinc-500 text-xs">
              ...
              <div className="flex-1 grid grid-cols-3 gap-4">
                {visibleImages.map((img) => (
                  <div
                    ...
                    className="h-28 border border-zinc-700 hover:border-emerald-500 rounded-none overflow-hidden bg-zinc-900 cursor-zoom-in relative group transition-colors"
                  >
      ...
      <div className="space-y-6">
        ...
        <div className="bg-zinc-800 border border-zinc-700 p-5 rounded-none space-y-4">
          <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-400 font-mono flex items-center gap-1.5 pb-2.5 border-b border-zinc-700">
          ...
          <div className="space-y-3 text-xs leading-normal">
            <div className="flex justify-between py-1.5 border-b border-zinc-700">
              <span className="text-zinc-400 font-medium">Ubicación Física</span>
              <span className="font-bold text-zinc-200 text-right">{project.location || 'N/D'}</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-zinc-700 font-mono">
              <span className="text-zinc-400 font-medium">Coordenadas GPS</span>
              <span className="font-bold text-amber-400">{project.gps_coordinates || 'N/D'}</span>
            </div>
            <div className="flex justify-between py-1.5">
              <span className="text-zinc-400 font-medium">Capacidad Nominal</span>
              <span className="font-bold text-zinc-200">{project.capacity || 'N/D'}</span>
            </div>
          </div>
        </div>

        {/* Client Profile / Expediente */}
        <div className="bg-zinc-800 border border-zinc-700 p-5 rounded-none space-y-4 text-left">
          <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-400 font-mono flex items-center gap-1.5 pb-2.5 border-b border-zinc-700">
            <Briefcase className="h-4 w-4 text-emerald-400" /> Expediente del Cliente
          </h3>
          <div className="space-y-3 text-xs leading-normal">
            <div className="flex justify-between py-1.5 border-b border-zinc-700">
              <span className="text-zinc-400 font-medium">Razón Social</span>
              <span className="font-bold text-zinc-200 text-right">{project.clients?.name || 'N/D'}</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-zinc-700">
              <span className="text-zinc-400 font-medium">RNC / Cédula</span>
              <span className="font-bold text-zinc-200">{project.clients?.document_id || 'N/D'}</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-zinc-700">
              <span className="text-zinc-400 font-medium">Teléfono</span>
              <span className="font-bold text-zinc-200">{project.clients?.phone || 'N/D'}</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-zinc-700">
              <span className="text-zinc-400 font-medium">Dirección</span>
              <span className="font-bold text-zinc-200 text-right">{project.clients?.address || 'N/D'}</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-zinc-700">
              <span className="text-zinc-400 font-medium">Categoría</span>
              <span className="font-bold text-zinc-200">{project.clients?.category || 'N/D'}</span>
            </div>
            <div className="flex justify-between py-1.5">
              <span className="text-zinc-400 font-medium">Consumo Promedio</span>
        ...
        >
          <div className="relative max-w-4xl max-h-[90vh] flex flex-col justify-center items-center">
            <button
              type="button"
              onClick={() => { setZoomUrl(null); }}
              className="absolute -top-12 right-0 bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-white rounded-none p-2 transition-colors cursor-pointer"
            >
              <X className="h-5 w-5" />
            </button>
            <img
