...
export default function ActivityTab({ activityMemberFilter, setActivityMemberFilter, employees, getProjectActivities }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-zinc-800 border border-zinc-700 p-4 rounded-none flex items-center gap-3">
        <span className="text-xs font-bold text-zinc-400">Filtrar por Miembro:</span>
        <select
      ...
      <div className="border border-zinc-700 rounded-none divide-y divide-zinc-700 max-h-96 overflow-y-auto text-left">
        {activities.map((act) => (
          <div key={act.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-zinc-900/40">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="bg-zinc-900 border border-zinc-700 text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-none text-zinc-450">
                  {act.action}
                </span>
              ...
              <p className="text-xs text-zinc-200 leading-normal">{act.details}</p>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <div className="h-6 w-6 rounded-none bg-zinc-900 border border-zinc-700 flex items-center justify-center text-[8px] font-bold text-zinc-300">
                {act.user_name?.charAt(0).toUpperCase()}
              </div>
              <span className="text-xs font-semibold text-zinc-400">{act.user_name}</span>
            </div>
          </div>
        ))}
